...
def processImage(folder, path):
    '''
    Iterate the GIF, extracting each frame.
    '''
    input_image = "%s/%s" % (folder, path)
    mode = analyseImage(input_image)['mode']
    ...
    try:
        while True:
            ...
            new_frame.save('%s/%s.png' % (folder, i+1), 'PNG')
            i += 1
